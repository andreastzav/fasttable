(function () {
  ...
  if (isWorkerContext) {
    ...
    const generationApi = self.fastTableGeneration;
    if (!generationApi) {
      self.postMessage({
        type: "workerInitError",
        error: "fastTableGeneration API unavailable in worker.",
      });
      return;
    }
  ...
  const generationApi = window.fastTableGeneration || {};
  ...
  function generateRowsWithWorkers(options) {
    ...
    return new Promise((resolve, reject) => {
      ...
      function dispatchNextChunk(slot) {
        ...
        if (nextChunkIndex >= chunkQueue.length) {
          if (slot.completedAtMs === null) {
            slot.completedAtMs = performance.now();
          }
          finishedWorkers += 1;
          resolveOnce();
          return;
        ...
        if (slot.startedAtMs === null) {
          slot.startedAtMs = performance.now();
        }
      ...
      for (let i = 0; i < workerSlots.length; i += 1) {
        ...
        let worker = null;
        try {
          worker = new Worker("./generation-workers.js");
        } catch (error) {
          rejectOnce(
            new Error(
      ...
      stallTimerId = setInterval(() => {
        ...
        const stalledForMs = performance.now() - lastProgressAtMs;
  ...
  window.fastTableGenerationWorkers = {
