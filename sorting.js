(function () {
  ...
  let cachedTimSortSortFn =
    typeof window !== "undefined" && typeof window.FastTimSort === "function"
      ? window.FastTimSort
      : null;
  let cachedTimSort0060SortFn =
    typeof window !== "undefined" &&
    typeof window.FastTimSort0060 === "function"
      ? window.FastTimSort0060
      : null;
  let cachedTimSort0018SortFn =
    typeof window !== "undefined" &&
    typeof window.FastTimSort0018 === "function"
      ? window.FastTimSort0018
      : null;
  let cachedQuadSortSortFn =
    typeof window !== "undefined" && typeof window.FastQuadSort === "function"
      ? window.FastQuadSort
      : null;
  let cachedFluxSortSortFn =
    typeof window !== "undefined" && typeof window.FastFluxSort === "function"
      ? window.FastFluxSort
      : null;
  ...
  function resolveTimSortSortFn() {
    if (typeof cachedTimSortSortFn === "function") {
      return cachedTimSortSortFn;
    }

    if (
      typeof window !== "undefined" &&
      typeof window.FastTimSort === "function"
    ) {
      cachedTimSortSortFn = window.FastTimSort;
      return cachedTimSortSortFn;
    }
  ...
  function resolveTimSort0060SortFn() {
    if (typeof cachedTimSort0060SortFn === "function") {
      return cachedTimSort0060SortFn;
    }
    ...
    if (
      ...
      cachedTimSort0060SortFn = window.FastTimSort0060;
      return cachedTimSort0060SortFn;
    }
  ...
  function resolveTimSort0018SortFn() {
    if (typeof cachedTimSort0018SortFn === "function") {
      return cachedTimSort0018SortFn;
    }
    ...
    if (
      ...
      cachedTimSort0018SortFn = window.FastTimSort0018;
      return cachedTimSort0018SortFn;
    }
  ...
  function resolveQuadSortSortFn() {
    if (typeof cachedQuadSortSortFn === "function") {
      return cachedQuadSortSortFn;
    }

    if (
      typeof window !== "undefined" &&
      typeof window.FastQuadSort === "function"
    ) {
      cachedQuadSortSortFn = window.FastQuadSort;
      return cachedQuadSortSortFn;
    }
  ...
  function resolveFluxSortSortFn() {
    if (typeof cachedFluxSortSortFn === "function") {
      return cachedFluxSortSortFn;
    }

    if (
      typeof window !== "undefined" &&
      typeof window.FastFluxSort === "function"
    ) {
      cachedFluxSortSortFn = window.FastFluxSort;
      return cachedFluxSortSortFn;
    }
  ...
  function sortRowsTimSort(rows, compareFn) {
    if (!Array.isArray(rows) || rows.length <= 1 || typeof compareFn !== "function") {
      return;
    }

    const timSortSort = resolveTimSortSortFn();

    if (timSortSort) {
      timSortSort(rows, compareFn);
      return;
    }

    throw new Error("TimSort mode requested but TimSort API is unavailable.");
  }

  function sortRowsTimSort0060(rows, compareFn) {
    if (!Array.isArray(rows) || rows.length <= 1 || typeof compareFn !== "function") {
      return;
    }

    const timSortSort = resolveTimSort0060SortFn();
    ...
    throw new Error(
      "TimSort 0060 mode requested but TimSort 0060 API is unavailable."
    );
  }

  function sortRowsTimSort0018(rows, compareFn) {
    if (!Array.isArray(rows) || rows.length <= 1 || typeof compareFn !== "function") {
      return;
    }

    const timSortSort = resolveTimSort0018SortFn();
    ...
    throw new Error(
      "TimSort 0018 mode requested but TimSort 0018 API is unavailable."
    );
  }

  function sortRowsQuadSort(rows, compareFn) {
    if (!Array.isArray(rows) || rows.length <= 1 || typeof compareFn !== "function") {
      return;
    }

    const quadSortSort = resolveQuadSortSortFn();

    if (quadSortSort) {
      quadSortSort(rows, compareFn);
      return;
    }

    throw new Error("QuadSort mode requested but QuadSort API is unavailable.");
  }

  function sortRowsFluxSort(rows, compareFn) {
    if (!Array.isArray(rows) || rows.length <= 1 || typeof compareFn !== "function") {
      return;
    }

    const fluxSortSort = resolveFluxSortSortFn();

    if (fluxSortSort) {
      fluxSortSort(rows, compareFn);
      return;
    }

    throw new Error("FluxSort mode requested but FluxSort API is unavailable.");
  }

  sortMethodRegistry.native = sortRowsNative;
  sortMethodRegistry.timsort = sortRowsTimSort;
  ...
  function registerSortMethod(mode, method) {
    ...
    const normalized = mode.trim();
    sortMethodRegistry[normalized] = method;
    return true;
  }
  ...
  function createSortController(options) {
    ...
    return {
      ...
      sortRows(rows, mode, sortOptions) {
        ...
        return {
          changedOrder: Array.isArray(rows) && rows.length > 1,
      ...
      sortIndices(indices, rowsByIndex, mode, sortOptions) {
        ...
        return {
          changedOrder: Array.isArray(indices) && indices.length > 1,
          durationMs: performance.now() - startMs,
          sortMode: plan.sortMode,
