(function () {
  ...
  function getActiveFilters(rawFilters) {
    const entries = getActiveFilterEntries(rawFilters);
    const activeFilters = {};

    for (let i = 0; i < entries.length; i += 1) {
      const entry = entries[i];
      activeFilters[entry.key] = entry.search;
    }
  ...
  function sortFilterKeysBySelectivity(filterKeys, activeFilters) {
    return filterKeys.slice().sort(function (a, b) {
      return activeFilters[b].length - activeFilters[a].length;
    });
  }
