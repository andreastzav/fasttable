...
function compareDictionaryKeyValuesAsc(aKey, bKey, columnKey) {
  ...
  if (valueType === "number") {
    const numericCompared = compareSortValuesAsc(
      Number(aKey),
      Number(bKey),
      "float"
    );
    if (numericCompared !== 0) {
      return numericCompared;
    }
  } else {
    const textCompared = compareSortValuesAsc(aKey, bKey, "string");
    if (textCompared !== 0) {
      return textCompared;
    }
  }
...
function buildSortedIndexColumnFromLowerDictionary(
  ...
  const sortedKeys = keys.slice().sort((a, b) =>
    compareDictionaryKeyValuesAsc(a, b, columnKey)
  );
...
function buildSortedIndexColumnFromDictionaryIds(
  ids,
  dictionary,
  columnKey,
  rowCount
  ...
  for (let i = 0; i < count; i += 1) {
    const id = ids[i];
    const dictValue = dictionary[id];
    if (valueType === "number") {
      values[i] = Number(dictValue);
    } else {
      values[i] = dictValue === undefined || dictValue === null ? "" : String(dictValue);
    }
  }

  return buildSortedIndexColumn(values, valueType === "number" ? "float" : "string", count);
}
...
function ensureNumericColumnarSortedIndices(numericColumnarData) {
  ...
  for (let colIndex = 0; colIndex < baseCount; colIndex += 1) {
    ...
    if (hasDictionary) {
      const sortedFromLowerDictionary = buildSortedIndexColumnFromLowerDictionary(
        lowerDictionaries[colIndex],
        columnKeys[colIndex],
        rowCount
      );
      if (
        sortedFromLowerDictionary instanceof Uint32Array &&
        sortedFromLowerDictionary.length === rowCount
      ) {
        sortedColumns[colIndex] = sortedFromLowerDictionary;
        continue;
      }

      sortedColumns[colIndex] = buildSortedIndexColumnFromDictionaryIds(
        columns[colIndex],
        dictionary,
        columnKeys[colIndex],
        rowCount
    ...
    const values = columns[colIndex];
    sortedColumns[colIndex] = buildSortedIndexColumn(
      values,
  ...
  const sortedByKey = Object.create(null);
  for (let colIndex = 0; colIndex < baseCount; colIndex += 1) {
    const sortedIndices = sortedColumns[colIndex];
    if (sortedIndices instanceof Uint32Array) {
      sortedByKey[columnKeys[colIndex]] = sortedIndices;
    }
  }
